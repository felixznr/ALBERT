...
Wenn jedoch anstatt einer Abweichung des Winkels eine Sinusschwingung (Türkis) als Störung für $\psi$ angenommen wird, wie in Abbildung \ref{fig:matlab_simulation_results_4} und \ref{fig:matlab_simulation_results_5} dargestellt, zeigt sich, dass die Regelung mit Nachführung der $\mathbf{K}$-Matrix in Abbildung \ref{fig:matlab_simulation_results_4} deutlich besser in der Lage ist, die Störung zu eliminieren und die Winkel $\theta$ und $\psi$ zu stabilisieren als ohne Nachführung der $\mathbf{K}$-Matrix (rechts). Ohne Nachführung der $\mathbf{K}$-Matrix kommt es zu einer gedämpten Schwingung der Winkel, was auf eine suboptimale Einstellung des Reglers zurückzuführen ist.
\begin{figure}[H]
